...
public class HeaderPanel {
    ...
    public Element render(SystemSnapshot systemSnapshot) {
        ...
        String loadStr = loadAverage[0] < 0
            ? "Load: N/A"
            : String.format("Load: %.2f %.2f %.2f", loadAverage[0], loadAverage[1], loadAverage[2]);
        ...
        return panel("INFO",
            column(text("Uptime: " + uptime + "     " + loadStr),
                row(text("Tasks: " + systemSnapshot.processes().size() + "   ").fg(Color.WHITE),
                    text("Running: " + running + "   ").fg(Color.GREEN),
                    text("Sleeping: " + sleeping + "   ").fg(Color.BLUE), text("Zombie: " + zombie).fg(Color.RED))))
